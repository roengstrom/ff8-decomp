...
- **Start from human-written code.** When decomping a new function, first write it as a human developer would have — clean, idiomatic C with proper structs, meaningful variable names, and natural control flow. Use this as the starting point, then iterate toward matching. Don't start from raw pointer offsets or compiler-oriented patterns.
