...
|--------|---|-----|-------|------------|
| `SLUS_008.92` | 618 | 412 | 1030 | 60.0% |
| `menumain.ovl` | 116 | 83 | 199 | 58.3% |
| `menucfg.ovl` | 9 | 9 | 18 | 50.0% |
| `menupty.ovl` | 15 | 34 | 49 | 30.6% |
| `menusts.ovl` | 15 | 29 | 44 | 34.1% |
| `menuabl.ovl` | 8 | 9 | 17 | 47.1% |
| `menushop.ovl` | 14 | 40 | 54 | 25.9% |
| `menuext.ovl` | 17 | 30 | 47 | 36.2% |
| `menuitem.ovl` | 33 | 66 | 99 | 33.3% |
| `menumgc.ovl` | 24 | 27 | 51 | 47.1% |
| `menugf.ovl` | 8 | 14 | 22 | 36.4% |
| `menujnc2.ovl` | 51 | 35 | 86 | 59.3% |
...
| `field_init.bin` | 8 | 1 | 9 | 88.9% |
| `display_init.bin` | 4 | 7 | 11 | 36.4% |
| `field_engine.bin` | 204 | 429 | 633 | 32.2% |
| `battle_engine.bin` | 63 | 566 | 629 | 10.0% |
| `battle_render.bin` | 10 | 17 | 27 | 37.0% |
| `battle_code.bin` | 510 | 891 | 1401 | 36.4% |
| `field_engine_alt.bin` | 33 | 312 | 345 | 9.6% |
| **Total** | **1869** | **3117** | **4986** | **37.5%** |
...
When renaming a `func_` or `D_` symbol to a meaningful name:

1. **Rename in source files** — Update all `.c` and `.h` files that reference the symbol:
   ```bash
   find src include -name '*.c' -o -name '*.h' | xargs sed -i 's/OLD_NAME/NEW_NAME/g'
   ```

2. **Rename in `symbol_addrs.*.txt`** — Update the entry in the relevant
   `config/symbol_addrs.*.txt` file(s). Splat uses these to generate `.s` file
   labels, so this ensures INCLUDE_ASM functions pick up the new name on the
   next `make split`.

3. **Add to overlay `symbol_addrs` if cross-referenced** — If other overlays
   reference the symbol (check `config/undefined_syms_auto.*.txt`), add the
   new name to each overlay's `symbol_addrs.*.txt` so splat renames it there
   too.

4. **Add linker aliases if needed** — If a manual `undefined_syms.*.txt`
   exists for an overlay, add the new name there. For symbols that appear in
   both C code (new name) and INCLUDE_ASM `.s` files (old name during
   transition), both names must resolve to the same address.

5. **Clean build and verify**:
   ```bash
   rm -rf asm build && make split && make verify
   ```
   Use `rm -rf build` (not just `rm -rf asm`) to clear stale `.o` files that
   may reference the old symbol name.

**Do not** edit `undefined_syms_auto.*.txt` or `undefined_funcs_auto.*.txt`
directly — these are regenerated by splat on each `make split`.
